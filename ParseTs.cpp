...
vector<string> ParseTypescript::getExtensions() const
{
    return {".ts"};
}

bool ParseTypescript::isSource(const string &fileName) const
{
    if (fileName.find(".ts"))
        return true;
    return false;
...
string ParseTypescript::goToNextFunction(istream &currentFile, vector<string> &fileLines)
{
    string line;
    string lineNext;
...
bool ParseTypescript::containsAccessibility(const string &line)
{
...
string ParseTypescript::findNameMeaning()
{
    string doc = "/*\n *";
...
void ParseTypescript::findReturn(const string &function)
{
...
void ParseTypescript::findName(const string& function)
{
